...
module.exports = {
     list: function() {
         ...
         shell.exec(cmd, {
             ...
             if (code === 0) {
                 winston.info('emulator.create: success');
                 defer.resolve(output.trim().split('\n'));
             } else {
                 winston.error('emulator.create: error occured running', cmd);
                 defer.reject(output);
             }
         });
     ...
     create: function(name, options) {
         ...
         winston.debug('emulator.create:', cmd);
     ...
     start: function(name) {
         ...
         function checkBooted(port) {
             ...
             winston.debug('emulator.start: poll port', port);
         ...
         random_port(function(port) {
             ...
             winston.info('emulator.start:', cmd);
     ...
     stop: function(port) {
         ...
         shell.exec(cmd, {
             ...
             if (code === 0 && !/error/.test(output)) {
                 winston.info('emulator.stop: success');
                 defer.resolve();
             } else {
                 winston.error('emulator.stop: error', output);
                 defer.reject(output);
             }
         });
